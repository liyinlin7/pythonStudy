...
class excel_info(object):
    def __init__(self, name="testinfo.xlsx", xls_path=''):
        logging.info(sys.path[0])
        if sys.path[0].find("TestCase") != -1:
    ...
    def get_xlsinfo(self):
        # listkey = ['saction']#小动作
        # listkey = ['sort','minute','second','voice']#音频
        # listkey = ['sort','bigname','number','times','rule','start','end','actions']#动作
        # listkey = ['sort','zhen','hit','xy']#关键帧
        listkey = ['sort','name','zhen','origin']#帧-动作origin
        api_dic = {}
