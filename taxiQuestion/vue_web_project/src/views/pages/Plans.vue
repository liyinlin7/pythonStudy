...
export default {
    ...
    setup () {
        ...
        const getData = (pageSize, pageIndex) => {
                getResquest(`/plans/?page_size=${pageSize}&page_index=${pageIndex}`) // 使用 ` 反引号 然后 ${形参} 引用
                // getResquest('/plans/', { page_size: pageSize, page_index: pageIndex })
                .then(
                    (res) => {
                        // 2.1 列表数据
