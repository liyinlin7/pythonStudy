...
class TodaySeries_basketball(object):
    '''
        雷竞技的系列赛
    '''
    ...
    @count_time
    def today_series_id_1_basketball(self):
        '''
            当天系列赛所有玩法
        :param env_flag:
        :return:
        '''
        sql = '''
                 SELECT * FROM `sports-basketball`.ex_series  where source  in (6,7)  and p_id != 0 and match_start_time >= (UNIX_TIMESTAMP() - 3600*24)
        ...
        if len(series_ex_id_str) != 0:
            mark_sql = f'''
                                SELECT * FROM `sports-lottery-center`.market where level_id in {series_ex_id_str};
                            '''
            print(mark_sql)
            if self.env_flag == 0:
                market_res = self.do_mysql.select(cnn=self.cnn, sql=mark_sql, env_flag=self.env_flag)
            else:
                market_res = self.do_mysql.select_basketball(cnn=self.cnn_basketball, sql=mark_sql, env_flag=self.env_flag)
            err1 = self.today_option_basketball(market_res)
            err3 = self.today_market_type_option_status_basketball(market_res)
    ...
    def today_option_basketball(self, market_res):
        '''
            当天所有系列赛的option 表
        :param datas:
        :return:
        ...
        try:
            ...
            if self.env_flag == 0:
                ...
                option_res = self.do_mysql.select_basketball(cnn=self.cnn_basketball, sql=sql, env_flag=self.env_flag)
            option_option_option_status_5_null = set()
            option_option_option_is_winner_1_2_null = set()
            err_str = '篮球：当天所有系列赛的option表中(market_type_id)，'
    ...
    def today_market_type_option_status_basketball(self, datas):
        '''
        ...
        try:
            ...
            if self.env_flag == 0:
                ...
                option_res = self.do_mysql.select_basketball(cnn=self.cnn_basketball, sql=option_sql, env_flag=self.env_flag)
...
if __name__ == '__main__':
    t = TodaySeries_basketball(env_flag=1).today_series_id_1_basketball()
