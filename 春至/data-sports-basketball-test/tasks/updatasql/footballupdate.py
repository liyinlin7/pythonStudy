...
class FootballUpdate(object):
    '''
        足球数据库修改操作表
    '''
    def __init__(self, env_flag):
        self.PRO_config = {'host': '172.16.0.120',     # 172.16.0.120  服务器访问 , 47.242.35.120  外网访问
                  'user': 'root',
                  'password': '0TNU0ogYb7TcrCfs',    # 修改权限密码
                  'port': 3306,
                  'auth_plugin': 'mysql_native_password'
                  }
        self.TEST_config = {'host': '121.196.189.137',
                      'user': 'root',
                      'password': 'Winter123',
                      'port': 3306,
                      'auth_plugin': 'mysql_native_password'
    ...
    def update_football_mathc(self):
        sql = '''
            update  `sports-soccer-match`.ex_match set audit =3 where  match_start_time <= unix_timestamp()-3600*4  and audit=1  and p_id = 0  and source in (2,3);
        '''
