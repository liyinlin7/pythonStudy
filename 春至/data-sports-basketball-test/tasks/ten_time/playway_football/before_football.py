...
class BeforeFootball(object):
    ...
    def before_option_status_football(self):
        ex_match_sql = '''
            SELECT * FROM `sports-soccer-match`.ex_match where p_id  != 0 and audit not in (1,3) and source in (6,7)  and status  in (2);
        '''
        mathcs = self.do_mysql.select(cnn=self.cnn, sql=ex_match_sql, env_flag=self.env_flag)
        ex_mathc_ids = set()
        for i in mathcs:
        ...
        for i in ex_mathc_ids:
            market_id_sql = f'''
                                    SELECT * FROM `sports-rate-center`.option where market_id in 
