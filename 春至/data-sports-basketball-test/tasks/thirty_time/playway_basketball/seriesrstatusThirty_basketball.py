...
class SeriesStatausThirth_basketball(object):
    '''
        根据系列赛判断指数玩法和选项状态
    '''
    ...
    def series_status_market_status_onehours_basketball(self):
        '''
            赛程开始时间后6个小时
        :return:
        '''
        sql = '''
            SELECT * FROM `sports-basketball`.ex_series  where source  in (6,7)  and p_id != 0
        ...
        if len(level_id_tuple) != 0:
            play_sql = f'''
                SELECT * FROM `sports-lottery-center`.market where level_id in {series_ex_id_str} order by level_id desc;
            '''
            print(play_sql)
            if self.env_flag == 0:
            ...
            if len(err_market_status_1) > 0:
                err_str += f'篮球：赛程开始时间后6个小时,玩法状态还是1的数据为（market_type的market_id）：{err_market_status_1}'
            if len(err_market_status_2) > 0:
                err_str += f'篮球：赛程开始时间后6个小时,玩法状态还是2的数据为（market_type的market_id）：{err_market_status_2}'
            if len(err_market_status_3) > 0:
                err_str += f'篮球：赛程开始时间后6个小时,玩法状态还是3的数据为（market_type的market_id）：{err_market_status_3}'
            print(err_str)
            if len(err_str) > 0:
                return err_str
            else:
    ...
    def series_status_option_status_onehours_basketball(self):
        '''
            比赛开始时间后6小时
        :return:
        '''
        ...
        if len(level_id_tuple) != 0:
            play_sql = f'''
                ...
                SELECT id FROM `sports-lottery-center`.market where level_id in {series_ex_id_str} order by level_id desc);
            '''
            if self.env_flag == 0:
                play_datas = self.do_mysql.select(cnn=self.cnn, sql=play_sql, env_flag=self.env_flag)
            else:
            ...
            if len(err_option_status_1) > 0:
                err_str += f'篮球：赛程开始时间后6个小时,选项状态还是1的数据为（option的ex_id）：{err_option_status_1}'
            if len(err_option_status_2) > 0:
                err_str += f'篮球：赛程开始时间后6个小时,选项状态还是2的数据为（option的ex_id）：{err_option_status_2}'
            if len(err_option_status_3) > 0:
                err_str += f'篮球：赛程开始时间后6个小时,选项状态还是3的数据为（option的ex_id）：{err_option_status_3}'
            if len(err_str) > 0:
                print(err_str)
                return err_str
            else:
...
if __name__ == '__main__':
    ss = SeriesStatausThirth_basketball(1)
    # ss.series_status_market_status_onehours()
    ss.series_status_market_status_onehours_basketball()
    ss.series_status_option_status_onehours_basketball()