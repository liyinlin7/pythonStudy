...
class TodaySeriesTen_basketball(object):
    '''
        雷竞技的系列赛
    '''
    ...
    def today_series_id_basketball(self):
        '''
            当天赛程所有玩法
        :param env_flag:
        :return:
        '''
        sql = '''
                 SELECT * FROM `sports-basketball`.ex_series  where source=2  and p_id != 0 and match_start_time >= (UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE)))
        ...
        if len(datas) != 0:
            ...
            err1 = self.today_market_type_basketball(market_res)
    ...
    def today_market_type_basketball(self, market_res):
        '''
        ...
        try:
            ...
            market_id_list = set()
            err_str = '篮球：当天所有赛程的market_type表中(level_id)，'
            ...
            market_name_err_str = self.today_market_name_basketball(market_market_id_list)
            option_err_str = self.today_option_basketball(market_id_list)
            err = ''
    ...
    def today_market_name_basketball(self, datas):
        '''
        ...
        try:
            ...
            err_str = '篮球：当天所有赛程的market_name表中(market_id)，'
            old_err_str_len = len(err_str)
    ...
    def today_option_basketball(self, datas):
        '''
            当天所有赛程的option 表
        :param datas:
        :return:
        ...
        try:
            ...
            option_id_list = set()
            err_str = '篮球：当天所有赛程的option表中(market_type_id)，'
            old_err_str_len = len(err_str)
            ...
            option_name_err = self.today_option_name_basketball(option_id_list)
            if len(err_str) > old_err_str_len:
    ...
    def today_option_name_basketball(self, datas):
        '''
            option_name 表
        :param datas:
        :return:
        ...
        try:
            ...
            err_str = '篮球：当天所有系列赛的option_name表中(id)，'
            old_err_str_len = len(err_str)
...
if __name__ == '__main__':
    t = TodaySeriesTen_basketball(env_flag=0).today_series_id_basketball()
