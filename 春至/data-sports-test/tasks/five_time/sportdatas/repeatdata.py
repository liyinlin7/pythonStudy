...
class RepeaData(object):
    ...
    def repeatdata_team_hour(self):
        ...
        :return:
        '''
        cnn = DoMySql().connect_mysql(env_flag=self.env_flag)
        sql = '''
            SELECT m.id,m.match_start_time,mt.team_id  FROM `sports-soccer-match`.match as m left join 
            `sports-soccer-match`.match_team as mt on m.id = mt.match_id WHERE m.create_time > unix_timestamp()-3600*24 and m.status in (1,2,3,4,5,6);
        '''
        datas = DoMySql().select(cnn, sql, self.env_flag)
        lists = []
        series_ids = []
        # 遍历保存series_id
        None_team_match = set()
        ...
        if bool:
            ...
            # people = '@15657880727'
            msg = "已入库两条系列赛数据出现队伍一致并且开赛时间相隔在1小时内,存在问题系列赛为:" + str(err_map) + "------"
