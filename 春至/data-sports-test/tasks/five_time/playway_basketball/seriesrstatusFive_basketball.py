...
class SeriesStatausFive_basketball(object):
    ...
    def series_status_market_status_basketball(self):
        sql = '''
            SELECT * FROM `sports-basketball`.ex_series where source=2  and p_id != 0 and match_start_time > unix_timestamp()-3600*24*3 and status=10 order by ex_id desc;
        '''
        ex_id_set = set()
    ...
    def series_status_market_null_basketball(self):
        sql = '''
            SELECT * FROM `sports-basketball`.ex_series  where source=2  and p_id != 0 and match_start_time >= UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE))-3600*8 
        ...
        if len(level_id_tuple) != 0:
            ...
            for i in play_level_id_set:
                if i in ex_id_list:
                    ex_id_list.remove(i)
            if len(ex_id_list) > 0:
                err_str += f'篮球：沙巴已过审的赛程，无指数详情数据,赛程ex_id为：{ex_id_list}'
            print(err_str)
            if len(err_str) > 0:
                return err_str
            else:
