...
class SeriesStataus_football(object):
    '''
        根据系列赛判断指数玩法和选项状态
    '''
    ...
    def series_status_market_status_onehours(self):
        ...
        '''
        sql = '''
            SELECT * FROM `sports-soccer-match`.ex_match  where source=2  and p_id != 0
    ...
    def series_status_option_status_onehours(self):
        ...
        if len(level_id_tuple) != 0:
            play_sql = f'''
                ...
                SELECT id FROM `sports-rate-center`.market_type where level_id in {level_id_tuple} order by level_id desc);
            '''
            if self.env_flag == 0:
                play_datas = self.do_mysql.select(cnn=self.cnn, sql=play_sql, env_flag=self.env_flag)
            else:
    ...
    #     sql = '''
    #         SELECT * FROM `sports-soccer-match`.ex_match  where source=2  and p_id != 0
    #         and match_start_time + 3600*3 > UNIX_TIMESTAMP()
	# 		order by ex_id desc;
    #     '''
    #     ex_id_set = set()
    #     data = DoMySql().select(cnn=self.cnn, sql=sql, env_flag=self.env_flag)
