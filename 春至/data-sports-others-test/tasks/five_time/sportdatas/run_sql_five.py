...
class RunSqlFive(object):
    ...
    def sql_five(self):
        sql = '''
            SELECT * FROM `test-sport`.auto_sql_others where type = 1 and time = 5 and status is NUll;
        '''
        ...
        if err_str != '':
            ...
            err_msg = f"> 其他：##### 本次总共执行{str(len(datas))}条sql语句，其中错误{str(err_count)}条，错误sql为：" + err_str
    ...
    def sql_five_SH(self):
        ...
        '''
        sql = '''
            SELECT * FROM `test-sport`.auto_sql_others where type = 1 and time = 5 and status=4;
        '''
        datas = DoMySql().select(cnn=self.cnn, sql=sql, env_flag=self.env_flag)
        print(len(datas))
        err_count = 0
