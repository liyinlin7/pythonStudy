...
class SeriesStatausFive_108_110(object):
    '''
        根据赛程判断指数玩法和选项状态
    '''
    ...
    def series_status_market_status_108_110(self):
        sql = '''
            SELECT * FROM `sports-others`.ex_series where source=2  and p_id != 0 and sport_id in (108,109,110) and match_start_time > unix_timestamp()-3600*24*3 and status=3 order by ex_id desc;
        '''
        ex_id_set = set()
        ...
        if len(level_id_tuple) != 0:
            ...
            if len(err_market_status) > 0:
                err_str += f'排台棒：赛程已结束,玩法状态还是1的数据为（market_type的market_id）：{err_market_status}'
            print(err_str)
            if len(err_str) > 0:
                return err_str
            else:
    ...
    def series_status_market_null_108_110(self):
        sql = '''
            SELECT * FROM `sports-others`.ex_series  where source=2  and p_id != 0 and sport_id in (108,109,110) and match_start_time >= UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE))-3600*8 
            and match_start_time < UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE) + INTERVAL 1 DAY)-3600*8 and status not in (4,5,100) order by ex_id desc;
        '''
        ex_id_set = set()
        data = self.do_mysql.select_basketball(cnn=self.cnn_basketball, sql=sql, env_flag=self.env_flag)
        for i in data:
        ...
        if len(level_id_tuple) != 0:
            ...
            for i in play_level_id_set:
                if i in ex_id_list:
                    ex_id_list.remove(i)
            if len(ex_id_list) > 0:
                err_str += f'排台棒：沙巴已过审的赛程，无指数详情数据,赛程ex_id为：{ex_id_list}'
            print(err_str)
            if len(err_str) > 0:
                return err_str
            else:
...
if __name__ == '__main__':
    ss = SeriesStatausFive_108_110(1)
    ss.series_status_market_status_108_110()
