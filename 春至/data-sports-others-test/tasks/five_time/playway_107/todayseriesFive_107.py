...
class TodaySeries_107(object):
    ...
    @count_time
    def today_series_id_1_107(self):
        '''
            当天系列赛所有玩法
        :param env_flag:
        :return:
        '''
        sql = '''
                 SELECT * FROM `sports-others`.ex_series  where source=2  and p_id != 0 and sport_id = 107 and match_start_time >= (UNIX_TIMESTAMP() - 3600*24)
        ...
        if len(series_ex_id_str) != 0:
            ...
            err1 = self.today_option_107(market_res)
            err3 = self.today_market_type_option_status_107(market_res)
    ...
    def today_option_107(self, market_res):
        '''
            当天所有系列赛的option 表
        :param datas:
        :return:
        ...
        try:
            ...
            err_str = '网球：当天所有系列赛的option表中(market_type_id)，'
            ...
            if len(option_option_option_status_5_null) != 0:
                err_str += f'网球：指数详情同选项id，option_status为5，is_winner不等于（3, 2,4,5,6）的数据为：{str(option_option_option_status_5_null)}；\n'
            if len(option_option_option_is_winner_1_2_null) != 0:
                err_str += f'网球：指数详情同选项id，is_winner为3或2，option_status不等于5的数据为：{str(option_option_option_is_winner_1_2_null)}；\n'
    ...
    def today_market_type_option_status_107(self, datas):
        '''
        ...
        try:
            ...
            if len(option_err_list_1) != 0:
                err_str += f'网球：指数详情 option表中（market_type_id）同玩法market_status等于1，任一option_status不为1的数据为：{str(option_err_list_1)}\n；'
            if len(option_err_list_2) != 0:
                err_str += f'网球：指数详情 option表中（market_type_id）同玩法market_status等于2，全部option_status不为2的数据为：{str(option_err_list_2)}\n；'
            if len(option_err_list_3) != 0:
                err_str += f'网球：指数详情 option表中（market_type_id）同玩法id，market_status为3，全部一option_status不是3的数据为：{str(option_err_list_3)}； \n'
            if len(option_err_list_4) != 0:
                err_str += f'网球：指数详情 option表中（market_type_id）同玩法id，market_status为4，全部option_status不是4的数据为：{str(option_err_list_4)}； \n'
            if len(option_err_list_5) != 0:
                err_str += f'网球：指数详情 option表中（market_type_id）同玩法id，market_status为5，全部option_status均不是5的数据为：{str(option_err_list_5)}； \n'
            ...
            if len(market_status_4_err) != 0:
                err_str += f'网球：指数详情 option表中（market_type_id）同玩法id，全部option_status为4，market_status不是4的数据为：{str(market_status_4_err)}； \n'
            if len(market_status_5_err) != 0:
                err_str += f'网球：指数详情 option表中（market_type_id）同玩法id，任一option_status为5，market_status不是5的数据为：{str(market_status_5_err)}； \n'
            if len(err_str) > old_err_str_len:
                print(err_str)
                return err_str + '\n'
            else:
...
if __name__ == '__main__':
    t = TodaySeries_107(env_flag=1).today_series_id_1_107()
