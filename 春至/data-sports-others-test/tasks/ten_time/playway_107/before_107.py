...
class Before107(object):
    ...
    def before_option_status_107(self):
        ex_match_sql = '''
            SELECT * FROM `sports-others`.ex_series where p_id  != 0 and audit not in (1,3) and source = 2 and sport_id = 107 and status not in (1,7);
        '''
        ...
        # print(match_ex_id_tuple)
            ...
            if len(err_option_ex_id) != 0:
                print(f"网球：比赛已经开始，但是赛前盘的选项还是开盘（option_ex_id）：{err_option_ex_id}")
                return f"网球：比赛已经开始，但是赛前盘的选项还是开盘（option_ex_id）：{err_option_ex_id}"
            else:
                return ''
