...
class a_war(object):
    ...
    def statistics_a_war(self, team_id, series_id, stats_date, stats_count, special_stats, game_id=4, status=0, query_type=1, stats_type=1):
        '''
        ...
        if status == 0:
            ...
            base_url = "http://" + url + "/api/stats/battle/kog?game_id={}&time_stamp={}".format(game_id, time_stamp)
        else:
            url = "openapi.dawnbyte.com"
            base_url = "https://" + url + "/api/stats/battle/kog?game_id={}&time_stamp={}".format(game_id, time_stamp)
        headers = {"sign": "1234"}
        print(base_url)
        if query_type == 1:
            if stats_type == 1:
...
if __name__ == '__main__':
    ...
    team_id = [2221, 2230]  # query_type= 2 时必填
    ...
    a_war.statistics_a_war(status=status, team_id=team_id, series_id=series_id, stats_date=stats_date,
