...
def csgo_matchID(request_sg=1, flag=1):
    '''
    ...
    # sql = "select * FROM `data-center`.match where series_id in " \
    #       "(SELECT id FROM `data-center`.series where 1600963200 <=start_time and  start_time<=1603261671) and status != 4 and deleted != 2;"
    sql = "select * FROM `data-center`.match where series_id in " \
          "(SELECT id FROM `data-center`.series where (unix_timestamp()-3600*24*30) <= start_time and  start_time < unix_timestamp() and  status != 4 and deleted = 1);"
    sql = '''
         select * FROM `data-center`.match where series_id in 
          (SELECT id FROM `data-center`.series where  start_time >=1614672000  and  start_time < unix_timestamp() and  status != 4 and deleted = 1) and deleted=1;
    '''
    ...
    if request_sg != 0:
        for i in ids:
            ...
            if bool:
                logging.info("系列赛ID" + str(ex_id) + "请求CSGO补充赛果接口成功")
    print(mactch_id)
...
if __name__ == '__main__':
    csgo_matchID(request_sg=1, flag=1)  # flag 0是测试数据库  request_sg: 0是不请求补充赛果接口
