        ...
        while True:
            try:
                ...
                if data not in illegal_str:
                    try:
                        res = eval(data)
                        if res['type'] == 301 and res['game_id'] == 4:
                            print("实时数据", mes)
                            # storage_data(data, con)
                        if res['type'] == 302 and res['game_id'] == 4:
                            print("实时统计", mes)
                            # storage_statistics(data, con)
                        if res['type'] == 303 and res['game_id'] == 4:
                            print("实时事件", mes)
                            # storage_event(data, con)
                        if res['type'] == 304 and res['game_id'] == 4:
                            print("实时特殊事件", mes)
                        if res['type'] == 401 and res['game_id'] == 4:
                            print("赛果数据", mes)
                        if res['type'] == 402 and res['game_id'] == 4:
                            print("赛果事件", mes)
                        if res['type'] == 403 and res['game_id'] == 4:
                            print("赛果特殊事件", mes)
