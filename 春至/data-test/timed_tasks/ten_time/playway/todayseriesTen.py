...
class TodaySeriesTen(object):
    ...
    def today_series_id(self):
        ...
        sql = '''
             ...
             '''
        # sql = '''
        #                 SELECT * FROM `data-center`.ex_series where deleted=1 and p_id != 0 and source =1 and start_time> 1613631600;
        #             '''
    ...
    def today_market_name(self, datas):
        ...
        try:
            market_name_sql = '''
                ...
                '''.format(tuple(datas))
            market_name_res = DoMySql().select(cnn=self.cnn, sql=market_name_sql, env_flag=self.env_flag)
    ...
    def today_option_name(self, datas):
        ...
        try:
            sql = '''
                        ...
                        '''.format(tuple(datas))
            option_name_res = DoMySql().select(cnn=self.cnn, sql=sql, env_flag=self.env_flag)
...
if __name__ == '__main__':
    t = TodaySeriesTen(env_flag=1).today_series_id()
